...
reading exposure**: once a day, surface a short A1 German text that naturally
contains the words the user is currently struggling with (Leitner box 1), with
the text's target words highlighted and tap-to-gloss.

Two constraints shape the design:

- **Invariant 1 — zero runtime LLM.** Generating natural A1 prose wants an LLM,
...
1. **Author (one-time, agent).** From a compact projection of `words.json`, the
   agent writes 50–80 A1 exam-style texts into `data/sources/daily-texts.src.json`,
   recording for each text the **exact surface form** of every embedded word
   (`used: [{ wordId, surface }]`). This is the only LLM step — same precedent as
   ADR 002's flagged-row cleanup / `en`-fill. Not deterministic; done once and
   reviewed.
...
dedicated `/read` route to reopen today's pick or browse all texts by topic. Both
read the same cached pick so they agree.
