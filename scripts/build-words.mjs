...
// Explicit irregulars/short forms not caught by the suffix ruleset below
// (comparative forms genuinely exist for all of these — that's what earns a
// word a spot here rather than in FORCE_OTHER).
...
// Reliable German adjective-forming suffixes — checked only after the noun
// (article), FORCE_OTHER, and ADVERBS passes, so an explicit override (e.g.
// 'hoffentlich' as adverb) always wins over the suffix guess.
const ADJECTIVE_SUFFIX_RE = /(ig|lich|isch|bar|sam|los|haft|mäßig)$/;
...
// Words that would otherwise false-positive on another pass (prepositions,
// conjunctions, pronouns, prefixes, and stray inflected participles that end
// in -en/-rn but aren't verbs, adjectives, or adverbs — or capitalized
// pronouns/fragments that would otherwise trip the capitalized-noun fallback).
...
function inferPos(lemma, article) {
  ...
  if (FORCE_OTHER.has(lower)) return 'other';
  if (ADVERBS.has(lower)) return 'adv';
  if (ADJECTIVES.has(lower) || ADJECTIVE_SUFFIX_RE.test(lower)) return 'adj';
  // verb: a (multi-word) lemma whose last token is an infinitive. Strip a
  // trailing "(sich)" first so reflexive verbs like "anmelden (sich)" test
  // their actual infinitive, not the literal "(sich)".
  const stripped = lemma.replace(/\s*\(sich\)\s*$/i, '');
  ...
  // A capitalized lemma with no article is a noun the source rows left
  // article-less (professions given as "X/in" pairs, plain nouns with a
  // missing article column, …) — checked last so it never preempts the verb
  // pass above (e.g. "Gassi gehen", "Tennis spielen").
  if (/^[A-ZÄÖÜ]/.test(lemma)) return 'noun';
  return 'other';
}
